#!/usr/bin/env python3
"""Capture session state via the Memoric REST API.

Safety net for PreCompact and Stop hooks — reads the transcript JSONL,
extracts structured session state, and stores it in Memoric directly.

Used by:
  - PreCompact hook: Tags with "pre-compaction"
  - Stop hook:       Tags with "session-end"
...
log = logging.getLogger("memoric-capture")
log.setLevel(logging.DEBUG)
_handler = logging.StreamHandler(sys.stderr)
_handler.setFormatter(logging.Formatter("[memoric-capture] %(message)s"))
log.addHandler(_handler)

API_URL = "https://memoric.dev"
MAX_TAIL_LINES = 500
...
def store_memory(api_key: str, content: str, user_id: str, source: str) -> bool:
    """Store session state as a memory via the Memoric REST API."""
    body = {
        "content": content,
        "user_id": user_id,
        "metadata": {
